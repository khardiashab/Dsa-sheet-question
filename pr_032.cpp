...
// input parameters and modifies the array in-place according to the
// given conditions.
// Note: The generated output is 1 if you modify the given array
// successfully.
...
int threeWayPartition(int arr[], int size, int a[]){
    partition(arr, size, a[0]);
    partition(arr, size, a[1]);
...
int main(){
    ...
    int a[] = {1,2};
    int w = threeWayPartition(arr, size, a);
    cout<< w<<endl;
    return 0;
}